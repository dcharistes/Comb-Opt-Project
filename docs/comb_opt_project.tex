...
\maketitle
\begin{abstract}
The Generalized Vehicle Routing problem investigated in this project is the Single Commodity Formulation \cite{bektas2011}. This is a study on model optimization for this problem proposing a custom Branch and Cut algorithm, as well as heuristics. The final architecture of the custom Branch and Cut algorithm is then compared to the simple Branch and Bound algorithm and the state-of-the-art Gurobi solver for a computational analysis on execution time and nodes traversed in increasing problem order.
\end{abstract}
...
In this project, the GVR problem we are concerned with, consists of finding a set of routes for a number of vehicles with limited capacities on a graph with the vertices (customers) partitioned into clusters. Customers are assigned with given demands -through a problem instance generation algorithm-. The total cost of the travel has to be minimized and all demands to be met. 

\subsection{Project Objectives and Outcomes}
The project's objective is to examine various techniques on heuristic methods, cutting planes to find what works best in Generalized Vehicle Routing Problems. To do so the problem will be modeled and optimized in Pyomo to get a solution reference and a computational footprint from the Gurobi solver, and then dive deeper into the actual solving process. The problem will be remodeled in gurobiPy in order to extract and import the appropriate information to the custom branch and cut algorithm that will be implemented. Then the optimization process using efficient data structures, heuristics and cutting planes will take place. The computational reference to this optimization will be the simple branch and bound algorithm. 
...
Constraint (7) serves as the commodity flow balance equation, linking the binary routing variables ($x$) with the continuous flow variables ($f$). In this pickup scenario, it mandates that the load leaving a customer node must exceed the load entering it by exactly the amount picked up. This constraint is non-redundant to the degree constraints (2) and (3); while the latter ensure that clusters are entered and exited, they fail to prevent isolated subtours (disconnected loops such as $C_1 \to C_2 \to C_1$). By enforcing flow accumulation from a source (the depot) to a sink, Constraint (7) renders such isolated loops mathematically impossible. Furthermore, it works in parallel with Constraint (8) to couple routing decisions with capacity limits, ensuring the vehicle's accumulated load never exceeds $Q$.
...
\end{equation}
To balance the positive flow generated by the customers, the depot must exhibit a negative net flow equal to the total collected demand:
\begin{equation}
...
Constraint (8) is the Strengthened Validity Inequality. It physically links the routing and flow variables, and it "tightens" the mathematical model to help the solver run faster. 

The lower bound represents the minimum load condition for the pickup problem. If a vehicle traverses arc $(i,j)$, it implies the vehicle has just visited node i. Since this is a Pickup problem, the vehicle must have picked up the demand $q_{α(i)}$ at node i. Therefore, the load on the vehicle immediately after leaving node i ($f_{ij}$) must be at least equal to that pickup amount. The upper bound represents the Strengthened Capacity Limit relative to the vehicle. If the vehicle leaves node i carrying a load, and it just used up $q_α(i)$ of its space, we can mathematically tighten the upper bound to be $Q-q_{a(i)}$. This strengthened bound is better than the simple $0\le f_{ij}\le Q$ because it tightens the LP relaxation solution and also prunes a tree earlier in the process.
...
The output of \textit{read\_data\_gvrp} is engineered to map directly to the arguments of \textit{build\_gvrp\_model}, decoupling data parsing from mathematical modeling.

The function abstracts away the orthogonal geometry of the grid. By converting $(x, y)$ coordinates to simple integers, it allows \textit{build\_gvrp\_model} to define the set $V$ as a simple range, simplifying the indexing of variables $x_{ij}$ and $f_{ij}$.
...
\begin{itemize}
    \item \textbf{Relocation Intensity ($k_{max}$):} The inner loop terminates if $k$ exceeds the maximum defined intensity $k_{max}$. This indicates that the current solution $S$ cannot be improved even after applying the strongest defined perturbation.
...
The experiments are run on an AMD Ryzen 5 4500U CPU processor with integrated graphics processing unit. 
...
\section{Results}
The results are presented in the below figures as well as in the table \ref{tab:results} in more detail.
\subsection{Time Comparison}
The time graph in Fig. \ref{fig:time_comp} of the custom branch and bound identifies a lack of correlation between the order of the problem and the solving time. It indicates that the order of the problem is not entirely correlated to the problem's difficulty and by extent the solving time increase. The factors defining the problem's difficulty are more a complex polynomial relation that can be researched in future work. 

The same goes for the simple branch and bound although it is observed that the time is almost half the time of branch and cut outlining a major performance outcome for the implementation of cuts. It will be explained in more detail in Section \ref{sub:nodes_expl} as the nodes explored performance metric removes the instability of the computer's processing time due to other processes running. The objective value of each problem that the branch and cut-bound algorithms and Gurobi find are the same respectively.

\subsection{Nodes Explored}
\label{sub:nodes_expl}
The nodes explored (Fig. \ref{fig:nodes_comp}) was a driving performance metric to determine the performance of the custom branch and bound. Here the branch and bound algorithm with the the cut method implemented shows its strength. As we can see the nodes explored of the branch and cut algorithm are approx. 50\% less than the nodes the branch and bound explored without implementing any cuts, but only for the first 3 problems. In the last two problems, the simple branch and bound explored more than twice the nodes of the branch and cut. This shows that the strength of the cut is not linear, but polynomial. 

The early tree pruning can remove a number of nodes from the solution space explore while the simple branch and bound includes in the solving process. This marks a higher order polynomial relation of the simple branch and bound to the problem's difficulty alike the custom branch and cut which remains in a lower polynomial order.
...
\begin{table}[htbp][H]
    ...
    \begin{threeparttable}
        \caption{Comparison of Solving Methods}
        \label{tab:results}
        
        % Use \small or \footnotesize to make the table fit nicely
        \small 
        \setlength{\tabcolsep}{4pt} % Adjust spacing as needed
        ...
        \begin{tablenotes}
            \footnotesize
            \item \textbf{BnC:} Custom Branch \& Cut.
            \item \textbf{BnB:} Simple Branch \& Bound (no cuts).
            \item \textbf{Gur:} Gurobi Optimizer..
        \end{tablenotes}
