...
def read_data_gvrp(filename):
    ...
    while "Arcs:" not in lines[line_idx]:
        ...
        demand = int(round(float(parts[3])))
...
def gvrp_model_gurobi(filename):
    ...
    for i in range(N):
        ...
        if i == depot_id:
            ...
            model.addLConstr(f_out - f_in == 0.5 * demand * x_sum, name=f"Demand_{i}")
    ...
    return model, ub, lb, integer_var, num_vars, c, arc_list, arc_to_idx, vars_list
